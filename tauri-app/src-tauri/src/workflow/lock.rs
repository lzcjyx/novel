...
impl GenerationLock {
    pub fn acquire(db: &Database, project_id: &str, lock_type: LockType) -> Result<Self, String> {
        ...
        if try_acquire_impl(db, id, project_id) {
            Ok(Self {
                db: db as *const Database,
                project_id: project_id.into(),
                lock_id: id,
            })
        } else {
            Err(format!(
                "Lock {:?} busy for project {}",
                lock_type,
                &project_id[..8]
            ))
        }
...
impl Drop for GenerationLock {
    fn drop(&mut self) {
        release_impl(unsafe { &*self.db }, self.lock_id, &self.project_id);
    }
}
...
pub fn cleanup_stale_locks(db: &Database, timeout_secs: i64) {
    if let Ok(conn) = db.conn.lock() {
        ...
        let _ = conn.execute(
            "DELETE FROM advisory_locks WHERE acquired_at < ?1",
            params![cutoff_str],
        );
    }
}
