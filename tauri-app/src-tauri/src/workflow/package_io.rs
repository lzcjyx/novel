...
use crate::db::{
    context_rules::ContextRule, draft_alternatives::DraftCandidate, model_profiles::ModelProfile,
    prompt_presets::PromptPresetPackage,
};
...
pub fn import_project_package(db: &Database, package: &ProjectPackage) -> Result<String, String> {
    ...
    insert_project_runtime_assets(&tx, &imported_project_id, &plan_id_map, package)?;
    insert_bible_rows(&tx, &imported_project_id, &package.bible)?;
    tx.commit()
...
pub fn import_novel_bible_package(
    ...
    let summary = insert_bible_rows(&tx, target_project_id, package)?;
