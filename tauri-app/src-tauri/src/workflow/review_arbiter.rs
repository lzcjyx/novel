...
pub fn aggregate_reviews(reviews: &[AgentReview], quality_threshold: i32, max_revise_count: i32, revise_count: i32) -> ReviewAggregation {
    ...
    if reviews.is_empty() {
        return ReviewAggregation {
            ...
            publish_allowed: false, blocking_issue_count: 0,
            all_passed: true, safety_passed: true,
            reviews: vec![],
        };
    }

    // Robust blocking detection: parse as JSON array and count actual items
    let has_blocking = reviews.iter().any(|r| {
        count_json_items(&r.blocking_issues) > 0
    });

    let safety_failed = reviews.iter().any(|r| {
        r.agent_name == "safety_reviewer" && r.pass == Some(false)
    });
