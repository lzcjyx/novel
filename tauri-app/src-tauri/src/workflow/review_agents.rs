...
use crate::models::{Project, Chapter, ChapterVersion, AgentReview};
use crate::prompts;
...
pub async fn run_review_agents(
    ...
    async fn timed_review(
        provider: &dyn ModelClient, name: &str,
        text: &str, title: &str, canon: &CanonContext,
    ...
    let (a1, a2, a3, a4, a5, a6, a7) = tokio::join!(
        timed_review(provider, "continuity_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "character_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "plot_logic_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "pacing_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "style_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "safety_reviewer", &chapter_text, &chapter_title, canon),
        timed_review(provider, "publication_reviewer", &chapter_text, &chapter_title, canon),
    );

    let results = [a1, a2, a3, a4, a5, a6, a7];
    let agent_names = [
        "continuity_reviewer", "character_reviewer", "plot_logic_reviewer",
        "pacing_reviewer", "style_reviewer", "safety_reviewer", "publication_reviewer",
    ];
...
async fn run_single_review(
    ...
    let template = prompts::load_prompt(agent_name)
        .unwrap_or_else(|_| format!("You are {}. Review the chapter and output valid JSON.", agent_name));
    ...
    vars.insert("RECENT_SUMMARIES_JSON".into(), canon.previous_chapters_json.clone());
    vars.insert("WRITING_BRIEF_JSON".into(), canon.writing_brief_json.clone());
    vars.insert("PLOT_THREADS_JSON".into(), canon.active_plot_threads_json.clone());
    vars.insert("FORESHADOWING_JSON".into(), canon.unresolved_foreshadowing_json.clone());
    vars.insert("CHARACTERS_JSON".into(), canon.characters_json.clone());
    vars.insert("CHARACTER_STATES_JSON".into(), canon.character_states_json.clone());
    ...
    vars.insert("PROJECT_POLICY_JSON".into(), canon.project_policy_json.clone());
    ...
    vars.insert("ORGANIZATIONS_JSON".into(), canon.organizations_json.clone());
    vars.insert("ITEMS_JSON".into(), canon.items_json.clone());
    vars.insert("MAGIC_SYSTEMS_JSON".into(), canon.magic_systems_json.clone());
    vars.insert("TIMELINE_JSON".into(), canon.timeline_json.clone());

    let render_vars: HashMap<&str, String> = vars.iter().map(|(k,v)| (k.as_ref(), v.clone())).collect();
    let system_prompt = prompts::render_prompt(&template, &render_vars);
    ...
    let raw = provider.generate_text(&system_prompt, &user_prompt, 16384).await?;

    // Extract JSON from the response
    let output: serde_json::Value = {
        let raw = raw.trim();
        if let Ok(v) = serde_json::from_str(raw) { v }
        else {
            let cleaned = raw
    ...
    if score < 20 {
        eprintln!("[WARN] {} returned score={}, raw first 300 chars: {}", agent_name, score, &raw[..raw.len().min(300)]);
    }
