...
use crate::models::{Chapter, ChapterFile, ChapterPlan, ChapterVersion};
use rusqlite::params;
...
pub fn get_latest_version(
    db: &Database,
    chapter_id: &str,
) -> Result<Option<ChapterVersion>, String> {
    let conn = db.conn.lock().map_err(|e| format!("Lock: {}", e))?;
...
pub fn update_chapter_version_metadata(
    db: &Database,
    version_id: &str,
    metadata: &serde_json::Value,
) -> Result<(), String> {
    let metadata_json = serde_json::to_string(metadata)
        .map_err(|e| format!("Serialize chapter version metadata: {}", e))?;
    let conn = db.conn.lock().map_err(|e| format!("Lock: {}", e))?;
    conn.execute(
        "UPDATE chapter_versions SET metadata = ?1, updated_at = datetime('now') WHERE id = ?2",
        params![metadata_json, version_id],
    )
    .map_err(|e| format!("Update chapter version metadata: {}", e))?;
    Ok(())
}
    ...
    let plans = stmt
        .query_map(params![project_id], |row| {
            Ok(ChapterPlan {
                ...
                title: row.get(4)?,
                outline: row.get(5)?,
                pov_character_id: row.get(6)?,
                target_word_count: row.get(7)?,
                required_characters: row.get(8)?,
                required_locations: row.get(9)?,
                plot_goals: row.get(10)?,
                required_foreshadowing: row.get(11)?,
                status: row.get(12)?,
                metadata: row.get(13)?,
...
pub fn get_next_chapter_plan(
    db: &Database,
    project_id: &str,
) -> Result<Option<ChapterPlan>, String> {
    let conn = db.conn.lock().map_err(|e| format!("Lock: {}", e))?;
    let result = conn.query_row(
        "SELECT id, project_id, volume_id, sequence, title, outline, pov_character_id,
                target_word_count, required_characters, required_locations, plot_goals,
