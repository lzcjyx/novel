...
pub fn get_settings(db: &Database) -> Result<AppSettings, String> {
    ...
    for (key, value) in pairs {
        ...
        match key.as_str() {
            ...
            "embedding_dimension" => { if let Ok(d) = v.parse() { settings.embedding_dim = d; } }
            "embedding_provider" => settings.embedding_provider = v.to_string(),
            "embedding_base_url" => settings.embedding_base_url = v.to_string(),
            "quality_threshold" => { if let Ok(d) = v.parse() { settings.quality_threshold = d; } }
            "auto_publish" => { settings.auto_publish = v == "true"; }
            "max_revise_count" => { if let Ok(d) = v.parse() { settings.max_revise_count = d; } }
            "daily_target_words" => { if let Ok(d) = v.parse() { settings.daily_target_words = d; } }
            "data_dir" => settings.data_dir = v.to_string(),
            "debug_mode" => { settings.debug_mode = v == "true"; }
...
pub fn save_settings(db: &Database, settings: &AppSettings) -> Result<(), String> {
    ...
    save_setting(db, "embedding_model", &format!("\"{}\"", settings.embedding_model))?;
    save_setting(db, "embedding_dimension", &settings.embedding_dim.to_string())?;
    save_setting(db, "embedding_provider", &format!("\"{}\"", settings.embedding_provider))?;
    save_setting(db, "embedding_base_url", &format!("\"{}\"", settings.embedding_base_url))?;
    save_setting(db, "quality_threshold", &settings.quality_threshold.to_string())?;
    save_setting(db, "auto_publish", if settings.auto_publish { "true" } else { "false" })?;
    save_setting(db, "max_revise_count", &settings.max_revise_count.to_string())?;
    save_setting(db, "daily_target_words", &settings.daily_target_words.to_string())?;
    save_setting(db, "data_dir", &format!("\"{}\"", settings.data_dir))?;
    save_setting(db, "debug_mode", if settings.debug_mode { "true" } else { "false" })?;
    save_setting(db, "blog_provider", &format!("\"{}\"", settings.blog_provider))?;
