// Fix Save Chapter to use correct schema with CTE query
...
const saveCode = 'var writerResp = $input.first().json;\n' +
  ...
  '  "INSERT INTO chapters (project_id, chapter_plan_id, sequence, title, status, word_count, summary) VALUES (" +\n' +
  '  esc(proj.id) + ", " + esc(plan.id) + ", " + esc(seq) + ", " +\n' +
  '  esc(title) + ", \x27draft\x27, " + esc(wordCount) + ", " + esc(summary) +\n' +
  '  ") RETURNING id" +\n' +
  '), ins_ver AS (" +\n' +
  '  "INSERT INTO chapter_versions (chapter_id, project_id, version_number, version_type, title, body_markdown, summary, word_count)" +\n' +
  '  " SELECT id, " + esc(proj.id) + ", 1, \x27draft\x27, " + esc(title) + ", " + esc(content) + ", " + esc(summary) + ", " + esc(wordCount) + " FROM ins_ch RETURNING id, chapter_id" +\n' +
...
async function main() {
  ...
  if (eci) {
    eci.parameters.query = '={{ $json.sql }}';
    console.log('Fixed Execute Chapter Insert');
  }
