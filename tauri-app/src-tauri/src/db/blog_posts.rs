...
pub fn create_blog_post(
    db: &Database, project_id: &str, chapter_id: &str, provider: &str,
    title: &str, slug: &str, url: Option<&str>,
...
pub fn get_blog_posts(db: &Database, project_id: &str) -> Result<Vec<BlogPost>, String> {
    ...
    let posts = stmt.query_map(params![project_id], |row| {
        Ok(BlogPost {
            id: row.get(0)?, project_id: row.get(1)?, chapter_id: row.get(2)?, provider: row.get(3)?,
            external_post_id: row.get(4)?, title: row.get(5)?, slug: row.get(6)?, url: row.get(7)?,
            status: row.get(8)?, published_at: row.get(9)?, metadata: row.get(10)?,
